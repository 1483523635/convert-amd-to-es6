...
function convert(source, options) {
    ...
    var result = falafel(source, {
        ...
        if (isModuleDefinition(node)) {
            nestedModules.push(node);
            // set the main call expression here as well
            // falafel performs pre-traversal so the last module node
            // will be the main node
    ...
    // take out the last nested module as that will be the main one
    nestedModules.splice(-1, 1);
    ...
    nestedModules.forEach(function (node) {
        var nestedModuleDeps = node.arguments.length > 1 ? node.arguments[0] : null;
        var nestedModuleFunc = node.arguments[node.arguments.length > 1 ? 1 : 0];
        var hasDeps = nestedModuleDeps && nestedModuleDeps.elements.length > 0;
        if (hasDeps) {
            var nestedModulePaths = nestedModuleDeps.elements.map(function (nestedNode) {
                return nestedNode.source();
            });
            var nestedImportNames = nestedModuleFunc.params.map(function (param) {
                return param.name;
            });
            var nestedDependenciesMap = {};
            extend(nestedDependenciesMap, nestedModulePaths.reduce(function (obj, path, index) {
                obj[path] = nestedImportNames[index] || null;
                return obj;
            }, {}));
            var newImport = nestedModulePaths.map(function (path) {
                return 'import(' + path + ').then(' + nestedModuleFunc.source() + ')';
            });
            node.update(newImport);
        }
        else {
    ...
    var moduleCode = getImportStatements(dependenciesMap, options.star);
    // add modules code
 ...
 * @returns {string}
 */
function getImportStatements(dependencies, starImport) {
    var statements = [];
    ...
    functionExpression.body.body.forEach(function (node) {
        if (node.type === 'ReturnStatement') {
            if (node.argument.type === 'ObjectExpression') {
                var returnObject = node.argument;
                if (returnObject.properties) {
                    var functions = [];
                    var identifiers = [];
                    for (var i = 0; i < returnObject.properties.length; i += 1) {
                        var returnProperty = returnObject.properties[i];
                        if (returnProperty &&
                            returnProperty.value &&
                            returnProperty.value.type === 'FunctionExpression') {
                            functions.push('export function ' +
                                returnProperty.key.name +
                                returnProperty.source().replace(returnProperty.key.name + ': function', '') +
                                '\r\n');
                        }
                        else if (returnProperty &&
                            returnProperty.value &&
                            returnProperty.value.type === 'Identifier') {
                            if (returnProperty.value.name === returnProperty.key.name) {
                                identifiers.push(returnProperty.key.name);
                            }
                            else {
                                identifiers.push(returnProperty.value.name + ' as ' + returnProperty.key.name);
                            }
                        }
                    }
                    var exportIdentifiers = identifiers.join(', ');
                    node.update(functions.join('') + 'export { ' + exportIdentifiers + ' };');
                }
            }
            else {
