// import { convert as amdToEs6} from '../lib/amdtoes6'
const amdToEs6 = require("../lib/amdtoes6").convert;
var fs = require('fs');
...
makeTest('define-with-deps-return-object');
...
var makeImportStarTest = function (name) {

    var baseTestName = 'test ' + name.replace(/-/g, ' ');
    var expectedOutputFileName = readFile(name.replace(/-arrow$/, '') + '-import-star-expected');

    var makeImportStarTestCase = function (inputFilename) {
        return function (test) {
            test.equal(amdToEs6(readFile(inputFilename), {beautify: true, star: true}), expectedOutputFileName);
            test.done();
        };
    };

    exports[baseTestName + ' (import star)'] = makeImportStarTestCase(name);
    exports[baseTestName + ' (import star using arrow function)'] = makeImportStarTestCase(name + '-arrow');

};
makeImportStarTest('define-with-deps');
makeImportStarTest('define-no-deps');
